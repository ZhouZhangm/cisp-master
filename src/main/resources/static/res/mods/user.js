...
layui.define(['laypage', 'fly', 'element', 'flow'], function (exports) {
    ...
    gather.mine = function (index, type, url) {
        ...
        var view = function (res) {
            ...
            elemUCM.children().eq(index).find('ul').html(
                res.rows.length === 0 ? '<div class="fly-msg">没有相关数据</div>' : html);
        };
                ...
                if (type === 'collection') {
                    ...
                    fly.json(url, {}, function (res) {
                        ...
                        var rows = layui.sort(res.rows, 'collection_timestamp', 'desc')
                            ...
                            if (last >= rows.length) {
                                last = curr > 1 ? start + (rows.length - start - 1)
                                    : rows.length - 1;
                            }
    ...
    if ($('#LAY_coop')[0]) {
        ...
        var effectView = function (res) {
            ...
            $('#LAY_effect_count').html(
                '你共有 <strong style="color: #FF5722;">' + (res.count || 0)
                + '</strong> 笔合作授权订单');
        };
